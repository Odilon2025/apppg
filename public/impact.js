...
const AMCI_ATUAL = [17615.20,18496.00,18865.92,19243.24,19628.11,20020.67,21021.66,21442.09,21870.93,22308.35,22754.52,23892.25,24250.60,24614.36,24983.58];
...
function renderFront(){
  const head = '<div><span>Nível</span><span>Ref. (Tempo de carreira)</span><span>APPGG (Atual)</span><span>AMCI (Atual)</span><span>AMCI (PL 699/2026)</span></div>';
  const rows = APP.map((app,i)=>{
    const level = i < 6 ? 'I' : i < 11 ? 'II' : 'III';
    return `<div><b>${level}</b><span><strong>${i+1}</strong> <small class="career-time">(${CAREER_TIME[i]})</small></span><strong>${brl(app)}</strong><span class="amci-current">${brl(AMCI_ATUAL[i])}</span><span class="amci-target">${brl(AMCI[i])}</span></div>`;
  }).join('');
  $('frontTable').innerHTML = head + rows;

  const mobileHead = '<div><span>Ref. (Tempo)</span><span>APPGG (Atual)</span><span>AMCI (Atual)</span><span>AMCI (PL 699)</span></div>';
  const mobileRows = APP.map((app,i)=>{
    return `<div><div><strong>${i+1}</strong> <small class="career-time">(${CAREER_TIME[i]})</small></div><span>${brlMobile(app)}</span><span>${brlMobile(AMCI_ATUAL[i])}</span><b>${brlMobile(AMCI[i])}</b></div>`;
  }).join('');
